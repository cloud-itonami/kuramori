(ns kuramori.methods.social
  "social.cljc — kuramori DRY-RUN self-publication seed (ADR-2606272355, auto-seeded R0).

  This is a GENERIC self-publication projection (draft-observation-post below builds an
  app.bsky.feed.post-shaped record), mechanically derived from kuramori's own
...
(defn draft-observation-post
  "A dry-run observation post over kuramori's own domain (Warehouse intralogistics robotics — the standalone warehouse-handling body the roster GAP-mapped to `kuramori 倉守` (ADR-2606073001 §3, 積み下ろし…)."
...
(defn build-live
  ...
  [& _args]
  (throw (ex-info (str "kuramori R0: live social posting is Council Lv6+ + operator + "
